...
class PatchManager {
  constructor(
    ...
    private modifierCtx: ModifierIconContext,
    private modifierEl: ModifierIconHandler,
    private onInterfaceReady: Modding.ModContext['onInterfaceReady']
  ) {
...
class ModifierIconHandler {
  ...
  private fixTextContentImgTag(node: HTMLElement): void {
    ...
    // Extract all tiny icon img tags
    const imgMatches = newText.match(ModifierIconHandler.TINY_ICON_TAGS) || [];

    // Remove all tiny icon img tags from the text content
    for (const match of imgMatches) newText = newText.replace(match, '').trim();

    // Reinsert the tiny icon img tags as html
    node.innerHTML = imgMatches.join('');

    // Append the text content after the tiny icon img tags
    node.append(newText);
  }
}
