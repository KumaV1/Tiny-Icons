...
// TODO: CONSIDER FORE-GOING THE RESOLVER APPROACH...
// Well, it still fits well with "injecting property onto ConditionalModifier" objects...
// ^ but it just does not work well regarding the case of modified descriptions...
// ^ As for those btw, they could use the "TagAllocationMemoizer", patching the corresponding classes (e.g. Item or Synergy) to check a tinyIcons boolean on the item (or a map) regarding whether icons have been added to "_modifiedDescription", pre-pending the icons (and setting flag to true) if not yet the case
// THEN AGAIN... have a look at "SummoningSynergy"... that shit does not have a "_modifiedDescription" onto which the icons can be pre-pended...

// Maybe I have to modify the lang keys after all... granted, considering mods, I will ALSO have to modify the _customDescription at times...
// guess it may be fine at the end of the day, resolvers are meant to be intuitive yet explicit implementations after all, so them knowing about the common LANG_STRING_X formats the game uses on various cases may not be that much a problem after all
// Keep in mind, though, that this WILL mean once more to have to check how to avoid "modifiedDescription" breaking the icons...
...
// TODO: Wait, the below class should not require a "data" property, right? The resolvers should be agnostic after all. There simply need to be a "resolver manager" (which is the logic that triggers when registering data)
 ...
 */
// TODO: Call them "propagators" instead, whose job it is to place "tinyIcons" objects as properties on other objects. Patching will continue to do all the utilizing then
// ^ Technically, this also makes that whole "Adhering to user settings" (e.g. Astrology/Agility) easier
